...
import { ChangeDetectionStrategy, Component, effect, ElementRef, inject, signal, untracked, ViewChild, OnDestroy } from '@angular/core';
...
export class App implements OnDestroy {
                ...
                if (idx !== -1) {
                    ...
                    setTimeout(() => {
                        ...
                        if (target) {
                            ...
                            const wrapper = this.mainArea.nativeElement.querySelector('#canvas-wrapper') as HTMLElement;
                            while (el && el !== wrapper) {
                            ...
                            const centerY = ((screenH + topActionBuffer) - (totalRenderHeight * comparisonScale)) / 2 - ((offsetT - protrudingHeight) * comparisonScale);
    ...
    private updateTransform() {
        this.canvasTransform.set(`translate(${this.panX()}px, ${this.panY()}px) scale(${this.scale()})`);
    ...
    private updateActiveTimelineTracker() {
        ...
        for (let i = 0; i < boards.length; i++) {
            ...
            const diff = Math.sqrt(
                Math.pow(bCenterX - screenCenterX, 2) +
                Math.pow(bCenterY - screenCenterY, 2)
            );
    ...
    centerOnBoard(board: HTMLElement, animate = true) {
        ...
        const { targetPanX, targetPanY } = calculateCenteringCoordinates(
            viewW, viewH, bWidth, bHeight, offsetL, offsetT, startScale
        );
    ...
    addSpecificMonth(year: number, monthIdx: number) {
        ...
        const sortedMonths = [...this.store.months()].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
