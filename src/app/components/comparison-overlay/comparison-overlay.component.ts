...
@Component({
  ...
  host: {
    'class': 'absolute top-0 -translate-y-[80px] left-1/2 -translate-x-1/2 z-50 bg-white rounded-[40px] border-2 border-slate/10 p-12 flex overflow-hidden transition-all w-max animate-reveal-pulse shadow-2xl drop-shadow-2xl'
  },
...
export class ComparisonOverlayComponent {
  ...
  categories = computed(() => {
    ...
    const map = new Map<string, { label: string, color: string, baseVal: number, targetVal: number }>();

    base.assetCategories.forEach(c => {
      const key = c.label.toLowerCase().trim();
      map.set(key, { label: c.label, color: c.color, baseVal: c.assets.reduce((sum, a) => sum + a.val, 0), targetVal: 0 });
    });
