...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-200 flex items-center justify-center p-4">
      ...
      <div
        class="relative bg-white rounded-[32px] shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-8 w-full max-w-md animate-in fade-in zoom-in-95 duration-200"
        role="dialog"
        aria-modal="true"
      >
        <!-- Close Button -->
        ...
        <p class="text-[16px] text-gray-500 mb-10 font-medium tracking-normal leading-relaxed">
          How each weight of marble translates to currency
        </p>
        ...
        <div class="mt-8 pt-4 w-full flex flex-col gap-3 border-t border-gray-50">
          <button
            (click)="close()"
            class="w-full h-14 bg-slate-900 border-2 border-slate-900 text-white hover:bg-slate-800 rounded-2xl font-black tracking-widest uppercase text-[11px] transition-all shadow-lg active:scale-[0.98]"
          >
            Confirm Update
          </button>
          <button
            (click)="resetDemo()"
            class="w-full h-12 border-2 border-red-50 text-red-500 hover:bg-red-50 rounded-2xl font-black tracking-widest uppercase text-[10px] transition-all active:scale-[0.98]"
          >
            Reset to Demo Data
          </button>
        </div>
