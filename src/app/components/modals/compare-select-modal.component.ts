    ...
    <div class="fixed inset-0 z-200 flex items-center justify-center p-4 transition-all duration-300"
      ...
      <div class="bg-white rounded-[32px] shadow-2xl p-8 w-[400px] border border-gray-100 flex flex-col relative transition-all duration-300"
         ...
         <div class="flex flex-col gap-6">
            <div>
              <label for="baseMonthSelect" class="block text-[15px] font-bold text-gray-600 uppercase tracking-widest mb-2 ml-2">Base Month</label>
              <select id="baseMonthSelect" (change)="setBaseId($event)" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-[14px] font-bold text-slate focus:ring-2 focus:ring-inset focus:ring-slate/20 outline-none cursor-pointer">
            ...
            <div>
              <label for="targetMonthSelect" class="block text-[15px] font-bold text-gray-600 uppercase tracking-widest mb-2 ml-2">Compare With</label>
              <select id="targetMonthSelect" (change)="setTargetId($event)" class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-[14px] font-bold text-slate focus:ring-2 focus:ring-inset focus:ring-slate/20 outline-none cursor-pointer">
                 @for (m of reversedMonths(); track m.id) {
         ...
         <div class="flex mt-10 justify-end gap-3">
            <button (click)="close()" class="py-2.5 px-6 rounded-md text-[16px] font-bold text-gray-600 hover:text-gray-800 bg-transparent transition-colors interactive-element">Cancel</button>
            <button (click)="confirm()" class="py-2.5 px-6 rounded-md text-[16px] font-bold text-white bg-slate hover:bg-black transition-colors interactive-element" [disabled]="!baseId() || !targetId() || baseId() === targetId()" [class.opacity-50]="!baseId() || !targetId() || baseId() === targetId()">Compare</button>
         </div>
      </div>
    </div>
  `
